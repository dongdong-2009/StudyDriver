...
static int scull_getwritespace(struct scull_pipe *dev, struct file *filp)
{
    ...
    {   /* full */
        ...
        if (spacefree(dev) == 0)
        {
            /* 假如唤醒进程在 if 之后, schedule() 之前试图唤醒本进程,  我们是否会
             * 在这里丢失一次被唤醒的机会?
             * 答: 在这种情况下, 不会出现任何问题. 因为wake_up_interruptible() 会将task状态
             * 改为TASK_RUNNING, schedule() 会直接返回--尽管根本没有必要调schedule().
             */
            schedule();
        }
...
static int scull_read_p_mem(char *buf, char **start,
